...
# import DiT 
from utils_sedd_dreamcoder_informedAbstraction_fantasy_wakePredictive_dit_xattn import *
...
if __name__ == '__main__':
    ...
    clip_score_threshold_fantasy = 25.0 
    ...
    max_fantasies = batch_size * 1
    fantasy_batch_size = 128 # 32
    fantasy_tries = int(max_fantasies / fantasy_batch_size)
    fantasy_start_iter = 6 * 1000
    prediction_loss_start_iter = 6 * 1000
    ...
    hyperparam_dict['method'] = 'seddA_dreamcoder_Fant_WakePredFix2_voc' 
    ...
    hyperparam_dict['Fdelay'] = fantasy_start_iter 
    hyperparam_dict['Pdelay'] = prediction_loss_start_iter 
    ...
    results_sleep_ps_loss, results_wake_ps_loss = [], []
    ...
    fantasy_dataset = [] 
    fantasy_ratio_numerator = min(batch_size, len(dataset))
    ...
    while train_step < num_train_steps + train_steps_done:
        ...
        if sleep_mode == 1: # sleep mode - train FSQ 
            ...
            train_on_wake_for_prediction = False
            if (train_step > prediction_loss_start_iter) and (train_step % 2 == 0) and (len(wake_dataset) > 0):
                sleep_dataset = wake_dataset 
                train_on_wake_for_prediction = True 
            ...
            if train_on_wake_for_prediction:
            ...
            if train_on_wake_for_prediction:
                ...
                if (train_step < prediction_loss_start_iter):
        ...
        if sleep_mode == 2: # dream mode - train DiT
            ...
            if (sleep_mode_counter == 0) and (train_step > fantasy_start_iter):
                with torch.no_grad():
            ...
            # train on replays and fantasies according to data proportions
        ...
        if (train_step+1) % sampling_freq == 0: ## sample 
            ...
            # if (sleep_mode == 0) and (len(wake_visualize_dataset) > 0): # wake mode - visualize wake dataset 

            #     x, x_r, cap = wake_visualize_dataset[-1]
            #     cap = cap.split('/')
            #     cap = ''.join(cap)
            ...
            #     save_path = results_dir + 'trainStep=' + str(train_step) + '_sleepMode=' + str(sleep_mode) + '_caption=' + cap + '.png'
        ...
        if (train_step+1) % plot_freq == 0: ## save ckpt and plot losses
            ...
            if sleep_mode == 1: # sleep mode - plot for FSQ
                ...
                plt.legend()
                plt.title('final_val:{:.3f}'.format(results_test_items[-1]))
                ...
                ax[0,0].set_title('train:{:.3f} recon:{:.3f} compress:{:.3f} pred:{:.3f}'.format(results_train_loss[-1], results_recon_loss[-1], results_compress_loss[-1], results_sleep_ps_loss[-1]))
                ...
                if first_plot[sleep_mode]:
                    results_train_loss, results_recon_loss, results_compress_loss, results_sleep_ps_loss = [], [], [], []
            ...
            if sleep_mode == 2: # dream mode - plot for DIT 
                ...
                plt.legend()
                plt.title('final_val:{:.3f}'.format(results_num_fantasies[-1]))
