...
class DIT_Embedder(nn.Module):
    ...
    def get_time_embedding(self, t, d_model, maxval=10000): # t.shape: [batch_size]
...
class FSQ_Encoder_Embedding(nn.Module):
    def __init__(self, x_dim, condition_dim, max_seq_len, d_model, device):
        super().__init__()
...
class FSQ_Transformer(nn.Module):
    ...
    def encode(self, x, condition): # [b, max_seq_len, x_dim] NOTE that max_seq_len = num_latents per x
...
def init_fsq_encoder_transformer(x_dim, condition_dim, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, out_dim, device):
    ...
    ff = FeedForward(d_model, d_ff, dropout) # feed forward block for each encoder / decoder block
    embedder = FSQ_Encoder_Embedding(x_dim, condition_dim, max_seq_len, d_model, device) # embedder block to obtain sequence of embeddings from sequence of input tokens
...
# caller function to instantiate the transformer, using the defined hyperparams as input
def init_fsq_decoder_transformer(x_dim, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, out_dim, device):
    rope = RotaryPositionalEmbeddings(d_model, max_seq_len ** 2, device)
    attn = FSQ_MultiHeadAttention(n_heads, d_model, d_k, d_v, rope, dropout) # multi head attention block
    ff = FeedForward(d_model, d_ff, dropout) # feed forward block for each encoder / decoder block
    ...
    model = FSQ_Decoder_Transformer(embedder, decoder, d_model, out_dim) # a non-causal encoder-only transfomer
    # initialize params - Xavier initialization
    for p in model.parameters():
        if p.dim() > 1:
            nn.init.xavier_uniform_(p)
...
# class for Predictive Scorer Embedding
class Predictive_Scorer_Embedding(nn.Module):
    def __init__(self, x_dim, condition_dim, max_seq_len, d_model, device):
        super().__init__()
        self.x_emb = nn.Linear(x_dim, d_model, bias=False)
        self.condition_emb = nn.Linear(condition_dim, d_model, bias=False)
...
# class implementing Predictive Scorer Transformer 
class Predictive_Scorer_Transformer(nn.Module):
    def __init__(self, embedder, encoder, d_model, out_dim):
        super().__init__()
        self.embedder = embedder 
        self.encoder = encoder
    ...
    def forward(self, x, condition): # x.shape: [batch_size, max_seq_len, x_dim]
        x_emb, condition_emb = self.embedder(x, condition) # x_emb.shape: [batch_size, max_seq_len, d_model]
        encoder_out = self.encoder(x_emb, condition=condition_emb) # encoder_out.shape: [batch_size, max_seq_len, d_model]
        encoder_out = encoder_out[:, 0] # encoder_out.shape: [batch_size, d_model]
        score = self.out_proj(encoder_out) # score.shape: [batch_size, out_dim]
        return score 
    

# caller function to instantiate the predictive scorer transformer
def init_predictive_scorer(x_dim, condition_dim, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, out_dim, device):
    rope = RotaryPositionalEmbeddings(d_model, max_seq_len ** 2, device)
    attn = FSQ_MultiHeadAttention(n_heads, d_model, d_k, d_v, rope, dropout) # multi head attention block
    ff = FeedForward(d_model, d_ff, dropout) # feed forward block for each encoder / decoder block
    embedder = Predictive_Scorer_Embedding(x_dim, condition_dim, max_seq_len, d_model, device) # embedder block to obtain sequence of embeddings from sequence of input tokens
    encoder_layer = FSQ_EncoderLayer(deepcopy(attn), deepcopy(attn), deepcopy(ff), d_model, dropout) # single encoder layer
    encoder = FSQ_Encoder(encoder_layer, n_layers, d_model) # encoder = stacked encoder layers
    model = Predictive_Scorer_Transformer(embedder, encoder, d_model, out_dim) # a non-causal encoder-only transfomer
    # initialize params - Xavier initialization
    for p in model.parameters():
        if p.dim() > 1:
            nn.init.xavier_uniform_(p)
